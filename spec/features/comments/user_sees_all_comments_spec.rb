...
describe 'User sees all comments for a job' do
  describe 'when user visits job/show page' do
    it 'shows all comments for a job' do
      ...
      job = company.jobs.create!(title: "Astronaut", level_of_interest: 80, city: "Denver", category_id: category.id)
      ...
      expect(page).to_have content(comment_1)
      expect(page).to_have content(comment_2)
    end
  end
end
